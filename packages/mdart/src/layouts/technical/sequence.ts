...
import { escapeXml, tt, wrapLabel, renderEmpty, parseLink, aWrap, itemTitleTag, ellipsisIfDropped, shouldAnimate, seqSpotlightCSS, wrapItem, shouldInstrument, FONT_SANS_ATTR } from '../shared'
...
export function render(spec: MdArtSpec, theme: MdArtTheme): string {
  ...
  actorRenders.forEach(({ actDisplay, lines, url: actUrl, tip, truncated }, i) => {
    ...
    unit.push(aWrap(`<text x="${x.toFixed(1)}" y="${textStartY.toFixed(1)}" text-anchor="middle" font-size="11" fill="${theme.text}" ${FONT_SANS_ATTR} font-weight="600">${fullTip}${spans}</text>`, actUrl))
  ...
  messages.forEach((msg, mi) => {
    ...
    if (isSelf) {
      ...
      const unit = [
        ...
        msg.msg ? `<text x="${(x1 + 4).toFixed(1)}" y="${(y - 4).toFixed(1)}" text-anchor="start" font-size="9" fill="${theme.textMuted}" ${FONT_SANS_ATTR}>${tt(msg.msg, maxCharsLoop)}</text>` : '',
      ].join('')
      ...
      const unit = [
        ...
        msg.msg ? `<text x="${midX.toFixed(1)}" y="${(y - 4).toFixed(1)}" text-anchor="middle" font-size="10" fill="${theme.textMuted}" ${FONT_SANS_ATTR}>${tt(msg.msg, maxChars)}</text>` : '',
      ].join('')
      parts.push(wrapItem(unit, n + mi, animate, instrument))
    }
  })
