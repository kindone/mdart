...
export function render(spec: MdArtSpec, theme: MdArtTheme): string {
  ...
  const W = 560, GAP = 6, HEADER_H = 32, ROW_H = 28
  ...
  shownItems.forEach((item, i) => {
    ...
    const { fontSize: headerFS, lineHeight: headerLH, results: [{ lines: headerLines, truncated: headerTruncated }] } =
      fitTextToWidthShared([itmDisplay], COL_W - 8, { maxSize: 10, minSize: 6.5, maxLines: 2, boxH: HEADER_H - 6 })
    ...
    const headerTextY = y + HEADER_H / 2 - ((headerLines.length - 1) * headerLH) / 2 + headerLH * 0.35
    ...
    item.children.slice(0, maxChildren).forEach((child, ci) => {
      ...
      const { fontSize: childFS, results: [{ lines: childLines, truncated: childTruncated }] } =
        fitTextToWidthShared([child.label], COL_W - 8, { maxSize: 9, minSize: 6, maxLines: 2, boxH: ROW_H - 4 })
      ...
      const childY = ry + ROW_H / 2 - ((childLines.length - 1) * (childFS * 1.3)) / 2 + childFS * 0.35
