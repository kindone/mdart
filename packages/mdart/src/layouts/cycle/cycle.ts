...
export function render(spec: MdArtSpec, theme: MdArtTheme): string {
  ...
  for (let i = 0; i < n; i++) {
    ...
    svgContent += renderFitBlock(nx, ny, fit, {
      ...
      shapeBounds: { x: nx - hw, y: ny - hh, w: NODE_W, h: NODE_H, label: 'cycle-node' },
    })
    svgContent += `</g>`
  }
  ...
  // Title in centre
  if (spec.title) {
    svgContent += `<text x="${cx}" y="${cy + 5}" text-anchor="middle" font-size="12" fill="${theme.textMuted}" ${FONT_SANS_ATTR}>${escapeXml(spec.title)}</text>`
  }
