...
export function render(spec: MdArtSpec, theme: MdArtTheme): string {
  ...
  parts.push(`<rect x="${TGT_X}" y="${(cy - tBH / 2).toFixed(1)}" width="116" height="${tBH}" rx="6" fill="${theme.accent}28" stroke="${theme.accent}" stroke-width="1.5">${itemTitleTag(target)}</rect>`)
  ...
  parts.push(aWrap(tgtContent, tgtUrl))
