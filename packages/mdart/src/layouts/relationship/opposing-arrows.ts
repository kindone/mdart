...
export function render(spec: MdArtSpec, theme: MdArtTheme): string {
  ...
  leftUnit.push(aWrap(renderWrappedText(leftCx, cy - 24, `text-anchor="middle" font-size="11" fill="${theme.text}" ${FONT_SANS_ATTR} font-weight="700"`, leftDisplay, leftLabelWrap, 12, left), leftUrl))
  let leftDetailY = cy + 4
  if (left.value) {
    const wrap = wrapLabel(left.value, 30, 2)
    leftUnit.push(renderWrappedText(leftCx, leftDetailY, `text-anchor="middle" font-size="9" fill="${theme.textMuted}" ${FONT_SANS_ATTR}`, left.value, wrap, 11))
    leftDetailY += wrap.lines.length * 11 + 2
  }
  left.children.slice(0, 3).forEach((ch) => {
    const wrap = wrapLabel(ch.label, 30, 1)
    leftUnit.push(renderWrappedText(leftCx, leftDetailY, `text-anchor="middle" font-size="8.5" fill="${theme.textMuted}" ${FONT_SANS_ATTR}`, ch.label, wrap, 10, ch))
  ...
  rightUnit.push(aWrap(renderWrappedText(rightCx, cy - 24, `text-anchor="middle" font-size="11" fill="${theme.text}" ${FONT_SANS_ATTR} font-weight="700"`, rightDisplay, rightLabelWrap, 12, right), rightUrl))
  let rightDetailY = cy + 4
  if (right.value) {
    const wrap = wrapLabel(right.value, 30, 2)
    rightUnit.push(renderWrappedText(rightCx, rightDetailY, `text-anchor="middle" font-size="9" fill="${theme.textMuted}" ${FONT_SANS_ATTR}`, right.value, wrap, 11))
    rightDetailY += wrap.lines.length * 11 + 2
  }
  right.children.slice(0, 3).forEach((ch) => {
    const wrap = wrapLabel(ch.label, 30, 1)
    rightUnit.push(renderWrappedText(rightCx, rightDetailY, `text-anchor="middle" font-size="8.5" fill="${theme.textMuted}" ${FONT_SANS_ATTR}`, ch.label, wrap, 10, ch))
