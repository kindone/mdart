...
export function render(spec: MdArtSpec, theme: MdArtTheme): string {
  ...
  if (centerItem) {
    ...
    parts.push(aWrap(`<text x="${cx}" y="${cy + 4}" text-anchor="middle" font-size="9" fill="${theme.text}" font-family="system-ui,sans-serif" font-weight="700">${itemTitleTag(centerItem)}${tt(ctrDisplay, 9, centerItem)}</text>`, ctrUrl))
  }
  items.slice(0, 4).forEach((item, i) => {
    ...
    parts.push(aWrap(`<text x="${px.toFixed(1)}" y="${(py - 7).toFixed(1)}" text-anchor="middle" font-size="11" fill="${theme.text}" font-family="system-ui,sans-serif" font-weight="700">${tt(itmDisplay, 13, item)}</text>`, itmUrl))
