...
export function render(spec: MdArtSpec, theme: MdArtTheme): string {
  ...
  for (let i = n - 1; i >= 0; i--) {
    ...
    unit.push(renderWrappedText(cx, rowY + 4, `text-anchor="middle" font-size="10.5" fill="${theme.text}" font-family="system-ui,sans-serif" font-weight="${i === 0 ? '800' : '650'}"`, itmDisplay, labelWrap, 12, item))
    if (item.value) {
