...
import { centeredTextY, displayLabel, fitTextToWidthShared, renderWrappedText, FONT_SANS_ATTR } from '../shared'
...
export function renderRelationshipBoxText(
  ...
  if (!item.value) {
    ...
    return renderWrappedText(cx, textY, `text-anchor="middle" font-size="${fit.fontSize}" fill="${theme.text}" ${FONT_SANS_ATTR} font-weight="${weight}"`, label, wrap, fit.lineHeight, item)
  }
  ...
  return renderWrappedText(cx, labelY, `text-anchor="middle" font-size="${labelFit.fontSize}" fill="${theme.text}" ${FONT_SANS_ATTR} font-weight="${weight}"`, label, labelWrap, labelFit.lineHeight, item)
    + renderWrappedText(cx, valueY, `text-anchor="middle" font-size="${valueFit.fontSize}" fill="${theme.textMuted}" ${FONT_SANS_ATTR}`, item.value, valueWrap, valueFit.lineHeight)
}
