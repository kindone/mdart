...
type AnsoffEntry = { display: string; url: string | null; src: ItemLike }
...
export function render(spec: MdArtSpec, theme: MdArtTheme): string {
  ...
  ANSOFF_QUADS.forEach((q, i) => {
    ...
    unit.push(`<text x="${x + CELL_W / 2}" y="${y + 24}" text-anchor="middle" font-size="11.5" fill="${text}" font-family="system-ui,sans-serif" font-weight="700">${escapeXml(q.label)}</text>`)
    unit.push(`<text x="${x + CELL_W / 2}" y="${y + 38}" text-anchor="middle" font-size="7.5" fill="${text}" font-family="system-ui,sans-serif" opacity="0.65">${q.sub}</text>`)
