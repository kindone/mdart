...
export function render(spec: MdArtSpec, theme: MdArtTheme): string {
  ...
  ANSOFF_QUADS.forEach((q, i) => {
    ...
    const headerFit = fitTextToWidthShared([q.label], CELL_W - 20, { maxSize: 11.5, minSize: 7, maxLines: 2, boxH: 28 })
    ...
    const subFit = fitTextToWidthShared([q.sub], CELL_W - 20, { maxSize: 7.5, minSize: 6, maxLines: 2, boxH: 20 })
    unit.push(renderWrappedText(
      x + CELL_W / 2,
