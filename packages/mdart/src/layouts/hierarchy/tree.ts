...
function renderNodeBox(node: RenderedNode, boxH: number, theme: MdArtTheme): string {
  ...
  return `<rect x="${bx.toFixed(1)}" y="${by.toFixed(1)}" width="${BOX_W}" height="${boxH}" rx="6" fill="${theme.accent}28" stroke="${theme.accent}" stroke-width="1.2">${itemTitleTag(node)}</rect>`
}
